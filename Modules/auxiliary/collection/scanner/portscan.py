...
class Module(Modules):
    Info = {
        ...
        "Options": (
            ...
            ("Proxy", "NO", True, 'Proxy Options', [
             "NO", "HTTP", "HTTPS", "SOCK5", "SOCK4", "SpiritProxy"]),
    ...
    def get_ip_status(self, ip, port):
        if self.Parameate["Proxy"] == "NO":
            ...
            server = socks.setdefaultproxy(
                socks.PROXY_TYPE_HTTP, self.Parameate["ProxyAddress"], int(self.Parameate["ProxyPort"]))
        try:
    ...
    def Exploit(self):
        ...
        for Ip in ScanHost:
            for port in Port:
                ...
                Count += 1
                t = threading.Thread(
                    target=self.scan, args=(str(Ip), int(port)))
                threads.append(t)
        for a in threads:
            a.start()
        for a in threads:
